...
module.exports.register = function (context, { config }) {
  ...
  context.on("navigationBuilt", ({ contentCatalog }) => {
    ...
    // Set md-url attribute on pages for <link rel="alternate"> in templates
    for (const page of allPages) {
      if (page.out && !page.asciidoc.attributes["page-llms-ignore"]) {
        const mdPath = page.out.path.replace(/\.html$/, ".md");
        page.asciidoc.attributes["page-md-url"] = `${siteUrl}/${mdPath}`;
      }
    }
