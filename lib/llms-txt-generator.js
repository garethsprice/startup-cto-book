...
/**
 * Antora extension that generates llms.txt and llms-full.txt files
 ...
 * - Anchor link cleanup in full-text output
 */
module.exports.register = function (context, { config }) {
  ...
  context.on("navigationBuilt", ({ contentCatalog }) => {
    ...
    function processPage(page) {
      ...
      const linkLine = `- [${title}](${siteUrl}/${page.out.path})${description ? `: ${description}` : ""}\n`;
      ...
      let plainText = nhm.translate(page.contents.toString());
    ...
    logger.info("llms.txt and llms-full.txt generated.");
  });
};
